...
| code-34 | code | heroiclabs/nakama-common (Go module) | Apache-2.0 | Nakama server-runtime API for the Go match handler (server/runtime-go; pins are load-compatibility requirements read from the running server's build metadata — see go.mod) | **v1.44.2** (go.mod, matches nakama 3.37.0's own build) | n — license text retained | 2026-09-06 — LICENSE fetched and read at the pinned tag (raw.githubusercontent.com/heroiclabs/nakama-common/v1.44.2/LICENSE, complete standard Apache-2.0 text); added ahead of the Go-runtime landing |
| code-35 | code | google.golang.org/protobuf (Go module) | BSD-3-Clause | wire types shared with the Nakama host binary (server/runtime-go) | **v1.36.11** (go.mod, matches nakama 3.37.0's own build) | n — license text retained | 2026-09-06 — LICENSE fetched and read at the pinned tag (raw.githubusercontent.com/protocolbuffers/protobuf-go/v1.36.11/LICENSE, "Copyright (c) 2018 The Go Authors", complete standard BSD-3-Clause text); added ahead of the Go-runtime landing |
| code-36 | code (toolchain) | Go toolchain | BSD-3-Clause | builds the Nakama Go runtime plugin (server/runtime-go; built in golang:1.25.5 matching the host server's go1.25.5) | **1.25.5** (go.mod toolchain, matches nakama 3.37.0's build metadata) | n — license text retained | 2026-09-06 — go.dev/LICENSE fetched and read (BSD-3-Clause, "Copyright 2009 The Go Authors"; no separate patent grant verified — none claimed); added ahead of the Go-runtime landing |
