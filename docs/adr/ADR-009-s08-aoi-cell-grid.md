...
The implementation written for this spike — re-homed 2026-09-06 to `shared/aoi.ts` so the normative spec lives outside the throwaway spike carve-out — is deliberately authored as the **normative algorithm spec for the Phase-5 Go port** of the region-match interest manager: the port must reproduce it step for step (Decision 3).
...
| **U 250 m** | **0.18 / 0.74** | **0.81 / 2.06** | **2.10 / 4.13** | **23.74 / 41.06** |
...
| **C 1000 m** | 0.05 / 0.14 | 1.00 / 2.35 | 4.26 / 8.43 | 100.06 / 146.49 |
